...
use crate::{EdgelinkError, get_json_value};
use crate::utils::constants::{ENV_STR, FLOW_STR, ID_STR, NAME_STR, SUB_FLOW_TYPE, TAB_STR, TYPE_STR};
...
pub fn load_flows_json_value(root_jv: JsonValue) -> crate::Result<ResolvedFlows> {
    ...
    for jobject in all_values.iter() {
        if let Some(obj) = jobject.as_object() {
            if let (Some(ele_id), Some(type_value)) = (
                obj.get(ID_STR).and_then(parse_red_id_value),
                obj.get(TYPE_STR).and_then(|x| x.as_str()).map(|x| parse_red_type_value(x)),
            ) {
                match type_value.red_type {
                   value if TAB_STR ==value => {
    ...
    for node_id in node_topo_sort.dependency_sort().iter() {
        ...
        if let Some(node) = flow_nodes.remove(node_id) {
            log::debug!(
                ...
                node_id,
                node.get(NAME_STR).and_then(|x| x.as_str()).unwrap_or(""),
                node.get(TYPE_STR).and_then(|x| x.as_str()).unwrap_or("")
            );
    ...
    for (flow_ordering, flow) in sorted_flows.into_iter().enumerate() {
        ...
        flow_config.subflow_node_id = if flow_config.type_name == SUB_FLOW_TYPE {
            let key_type = format!("{SUB_FLOW_TYPE}:{}", flow_config.id);
            let node =
                all_values.iter().find(|x| x.get(TYPE_STR).and_then(|y| y.as_str()).is_some_and(|y| y == key_type));
            node.and_then(|x| x.get(ID_STR)).and_then(parse_red_id_value)
        } else {
            None
        };
...
fn preprocess_subflows(jv_root: JsonValue) -> crate::Result<JsonValue> {
    ...
    for jv in elements.iter() {
        if let Some(("subflow", subflow_id)) = jv.get(TYPE_STR).and_then(|x| x.as_str()).and_then(|x| x.split_once(':')) {
            let subflow = elements
                .iter()
                .find(|x| x.get(ID_STR).and_then(|y| y.as_str()).is_some_and(|y| y == subflow_id))
                .ok_or(EdgelinkError::BadFlowsJson(format!(
                    "The cannot found the subflow for subflow instance node(id='{}', type='{}', name='{}')",
                    subflow_id,
                    get_json_value!(jv,TYPE_STR,str),
                    get_json_value!(jv,NAME_STR,str),
                )))?;
    ...
    for pack in subflow_packs.iter() {
        ...
        // "subflow" element
        {
            ...
            new_subflow[ID_STR] = JsonValue::String(subflow_new_id.to_string());
            id_map.insert(pack.subflow_id.to_string(),subflow_new_id.to_string());
        ...
        // the fixed subflow instance node
        {
            ...
            new_instance[TYPE_STR] = JsonValue::String(format!("subflow:{}", subflow_new_id));
        ...
        for old_child in pack.children.iter() {
            ...
            new_child[ID_STR] = generate_new_xored_id_value(subflow_new_id, old_child[ID_STR].as_str().unwrap())?;
            id_map.insert(old_child[ID_STR].as_str().unwrap().to_string(), new_child[ID_STR].as_str().unwrap().to_string());
    ...
    for node in new_elements.iter_mut() {
        ...
        if let Some(JsonValue::String(pvalue)) = node.get_mut(TYPE_STR) {
            if let Some(("subflow", old_id)) = pvalue.split_once(':') {
                if let Some(new_id) = id_map.get(old_id) {
                    *pvalue = format!("{SUB_FLOW_TYPE}:{}", new_id);
                }
            }
        }
        ...
        if let Some(JsonValue::Array(in_props)) = node.get_mut("in") {
            for in_item in in_props.iter_mut() {
                for wires_item in in_item["wires"].as_array_mut().unwrap().iter_mut() {
                    if let Some(JsonValue::String(pvalue)) = wires_item.get_mut(ID_STR) {
                        if let Some(new_id) = id_map.get(pvalue.as_str()) {
                            *pvalue = new_id.to_string();
                        }
                    }
...
impl RedFlowJsonObject for JsonMap<String, JsonValue> {
    fn get_flow_dependencies(&self, elements: &[JsonValue]) -> HashSet<ElementId> {
        ...
        let related_link_in_ids = elements
            ...
            .filter_map(|x| {
                if x.get("z") == this_id
                    && (option_value_equals_str(&x.get(TYPE_STR), "link out")
                        || option_value_equals_str(&x.get(TYPE_STR), "link call"))
                {
        ...
        elements
            .iter()
            .filter(|x| {
                if let Some(flow_id) = x.get(ID_STR) {
    ...
    fn get_subflow_dependencies(&self, elements: &[JsonValue]) -> HashSet<ElementId> {
        let subflow_id = self.get(ID_STR).and_then(|x| x.as_str()).expect("Must have `id`");
        ...
        elements
            ...
            .filter(|o| {
                o.get(TYPE_STR)
                    ...
                    .is_some_and(|x| x.0 == SUB_FLOW_TYPE && x.1 == subflow_id)
            })
...
impl RedFlowNodeJsonObject for JsonMap<String, JsonValue> {
    fn get_flow_node_dependencies(&self) -> HashSet<ElementId> {
        ...
        if let Some(links) = self.get("links").and_then(|x| x.as_array()) {
            let red_type = self.get(TYPE_STR).and_then(|x| x.as_str());
...
impl RedPropertyType {
    pub fn from(ptype: &str) -> crate::Result<RedPropertyType> {
        match ptype {
            ...
            value if value== FLOW_STR => Ok(RedPropertyType::Flow),
...
fn preprocess_merge_subflow_env(flows: &mut JsonValue) -> crate::Result<()> {
    ...
    let subflows: HashMap<String, JsonValue> = elements
        .iter()
        .filter(|x| x.get(TYPE_STR).and_then(|y| y.as_str()).map(|y| y == SUB_FLOW_TYPE).unwrap_or(false))
        .filter(|x| x.get(ENV_STR).is_some())
        .map(|e| (e.get(ID_STR).and_then(|x| x.as_str()).unwrap().to_string(), e.get(ENV_STR).cloned().unwrap()))
        .collect();

    for element in elements.iter_mut() {
        if let Some(("subflow", subflow_id)) =
            element.get(TYPE_STR).and_then(|x| x.as_str()).and_then(|x| x.split_once(':'))
        {
            if let Some(subflow_env) = subflows.get(subflow_id) {
                let instance_env = if let Some(instance_env) = element.get_mut(ENV_STR) {
                    instance_env
                } else {
                    element[ENV_STR] = JsonValue::Array(Vec::new());
                    element.get_mut(ENV_STR).unwrap()
                };
...
fn merge_env(target_envs: &mut JsonValue, ref_envs: &JsonValue) -> crate::Result<()> {
    ...
    let target_names: HashSet<String> = target_vec
        .iter()
        .filter_map(|item| item.get(NAME_STR))
        .filter_map(|name| name.as_str())
        .map(|name| name.to_string())
        .collect();

    for item in ref_vec.iter() {
        if let Some(name) = item.get(NAME_STR).and_then(|name| name.as_str()) {
