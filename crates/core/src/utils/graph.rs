...
impl<N: Eq + Ord + Clone, E: Default + Clone> Graph<N, E> {
    ...
    pub fn path_to_top<'a>(&'a self, pkg: &'a N) -> Vec<(&'a N, Option<&'a E>)> {
        self.path_to(pkg, |s, pk| {
            ...
            // it's used for!
            s.nodes.iter().filter_map(|(p, adjacent)| adjacent.get(pk).map(|e| (p, e)))
        })
    }
}
        ...
        'a: 's,
    {
        ...
        #[cfg(debug_assertions)]
        {
            for x in result.windows(2) {
                let [(n2, _), (n1, Some(e12))] = x else { unreachable!() };
                assert!(std::ptr::eq(self.edge(n1, n2).or(self.edge(n2, n1)).unwrap(), *e12));
            }
...
fn path_to_case() {
    ...
    new.link(2, 0);
    new.link(2, 1);
    assert_eq!(new.path_to_bottom(&2), vec![(&2, None), (&0, Some(&())), (&3, Some(&()))]);
}
